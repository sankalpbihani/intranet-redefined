...
error_reporting(0);		?>
        ...
        if (isset($_POST['txtUserNm'])) {
            ...
            $pass = $_POST['txtPass'];
			//echo $pass;
			//$captcha = $_POST['captch'];
			echo $_SESSION["vercode"];
			echo $_POST["vercode"];
			//echo $captcha;
            if ((!preg_match(getUserNamePattern(), $user_nm)) || (!preg_match(getPasswordPattern(), $pass))||(!($_POST["vercode"]==$_SESSION["vercode"]))||$_SESSION["vercode"]=='') {
                $pageStatus = "INVALID";
            } else {
...
//        }
        if ($pageStatus == "REQUESTED") {
            ...
            if (!($con = mysql_connect(constant("HOSTNAME"), constant("USERNAME"), constant("PASS")))) {
                ...
                if($user_type=="std") {
                    $sql = "SELECT name,roll_number,class,pass_changed,permission,advisor_name,last_logged FROM student,advisor WHERE student.advisor_id=advisor.advisor_id AND user_nm='" . $user_nm . "' and password='" . $pass . "'";
                    $rs = mysql_query($sql);
            ...
            <div id="adminMiddle">
                <div id="adminMiddleContent" style="text-align: center;width: 400px;padding-top: 150px">
                    <form action="login.php" method="POST">
                        ...
                        <td>Enter Code <img src="login.php"><input type="text" name="vercode" value="" /></td>
							</tr>
                            <tr>
