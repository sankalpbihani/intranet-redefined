...
function news_init() {
	...
	elgg_extend_view('js/elgg', 'news/js');
	// routing of urls
	...
	// entity menu
	elgg_register_plugin_hook_handler('register', 'menu:entitynews', 'entitynews_menu_setup');
 ...
 */
function entitynews_menu_setup($hook, $type, $return, $params) {
...
if ($entity && elgg_instanceof($entity) && $entity->canEdit()) {
		...
		$delete = array(
			'name' => 'delete',
			'title' => elgg_echo('news:delete'),
			'text' => elgg_echo('news:delete'),
			'id' => "news-ajaxed-remove-{$entity->guid}",
			'class' => 'news-ajaxed-remove',
			'href' => elgg_add_action_tokens_to_url("action/$handler/delete?guid={$entity->getGUID()}"),
			'priority' => 900,
			'section' => 'dropdown'
		);
